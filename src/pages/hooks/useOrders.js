...
const uploadFinalVideo = async (orderId, file) => {
  try {
    ...
    // Remove order_id from formData since it needs to be in query params

    // Add order_id as query parameter in the URL
      ...
      prevOrders.map(order =>
        order.id === orderId
          ? {
              ...
              finalVideoUrl: result.final_video_url || result.video_url || result.local_url || result.url || order.finalVideoUrl,
            }
          : order
      )
    );
